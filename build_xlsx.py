...
rows1 = [
    ...
    ["Atilla es a Fehernep (jurtak.hu)", "Ungarn", "https://jurtak.hu/  |  https://fehernep.hu/",
     "4 / 6 / 8 / 10 m",
     "8m Frame-only KONKRET 25.05.2026: Buche-Basis 6.600 + EICHE-Upgrade 410 + OEL-Behandlung 340 = 7.350 EUR netto. Versand selbst-Spediteur OK.",
     "Rahmen WAEHLBAR: Eiche (Empfehlung, Algarve-optimal) statt Buche (Standard). Rest Sibirische Kiefer. ALLES OEL-BEHANDELT (atmungsaktiv, fuer Algarve perfekt!). Plane + Kuppel + Filz Sebastian besorgt selbst (Algarve-Sattler/Chris).",
     "Sehr gut: EICHE (Klasse 2 Dauerhaftigkeit, Termiten-resistent durch Gerbstoffe) + OEL (diffusionsoffen) = optimal fuer humide Atlantikkueste.",
     "Versand HU->Algarve via Sebastians eigener Spediteur (Atilla bestaetigt 'positive') ca. 800 EUR. Lieferzeit 2 Mon. Aufbau Sebastian selbst (mit 4-6 Helfern).",
     "Atilla Berki, Jurtenbauer seit 1996, erster kommerzieller HU-Hersteller. Sozial: Rahmen aus Therapieprogramm. Ehrlich+praezise Kommunikation. Kontakt: jurtak@gmail.com / +36 30 657 0740",
     "DIY-MODIFY 8M VOLLKOSTEN MIT ATILLA-EICHE: 7.350 Frame + 800 Versand + 2.500 Sauleda-Plane lokal + 1.000 Wolle + 700 Solitex + 800 Innenliner + 800 Kuppel + 200 Schrauben + 2.880 Plattform DIY + 500 Sturm-Kit + 240 Schatten + 500 Reserve = ~18.270 EUR. 100+ Std Eigenarbeit. PREIS-KOENIG fuer 8m wenn DIY-Lust + Cashflow-Tauglichkeit. Eiche statt Pinie/Buche ist STRUKTURELLER VORTEIL fuer Algarve."],
    ...
    ["Dani + Chris (Castelo de Vide Community-Projekt) - 7m Custom + TenCate Campshield Option",
     "Portugal - Castelo de Vide (Alto Alentejo, ca. 400 km von Aljezur). Dani=Frau (Kommunikation), Chris=Mann (technisch+Schafwolle)",
     "7m Innenflaeche ca. 38 m2, Mittelhoehe 360 cm, Dachneigung 28 Grad",
     "16.000 EUR inkl. Aufbau (Standard mit BW/Poly Canvas+4cm Wolle). Transport+Plattform NICHT inkl. OPTIONAL (Sebastian beschafft selbst): andere Stoffe bis 420g/m² (Dani naeht), dickere Daemmung. NICHT moeglich: zusaetzliche Fenster (Waende schon fertig).",
     "Khaana: PINIE-Scherengitter, 200cm Wandhoehe. Dachring (Tono): DOUGLASIE. Dachkuppel 150cm doppeltes Plexiglas (stufenlos oeffenbar, windfest). Tueren: 2 Hartholz-Doppeltueren 155x200cm zweifach verglast (= 4,8 m² Glasflaeche durch Tueren, plus 1,77 m² Skylight). KEINE zusaetzlichen Wandfenster moeglich. Daemmung Standard: 4cm Schafwolle. Innenstoff: 100% Biobaumwolltuch. Aussenstoff STANDARD: BW/Polyester 50/50 Canvas ueber ESVO NL.",
     "VERHANDELBAR: Dani naeht andere Stoffe bis 420g/m² wenn Sebastian sie SELBST besorgt+liefert (NICHT aktiv von Dani angeboten als Upgrade). Mein Vorschlag Sauleda Solar Pro/Tempotest waere Sebastians Eigeninitiative fuer Algarve-UV. Aehnlich: dickere Daemmung wenn Sebastian Material organisiert.",
     "Transport Castelo de Vide -> Aljezur: 800-1.500 EUR (zu klaeren - 'Aufbau' inklusive, aber wo? Bei Dani oder Sebastian?). Aufbau IM 16k-Preis. Plattform bauen sie KEINE (Sebastian baut lokal: 1.925 EUR EPS-Variante oder 2.880 EUR Kork-Variante, siehe Sheet 17).",
     ...
     "DREI VARIANTEN ALL-IN: 1) MINIMUM (Standard-Canvas, EPS-Plattform): 16k + 1k Transport + 1.925 + 240 = 19.165 EUR. 2) MITTEL (Standard-Canvas, Kork-Plattform): 16k + 1k + 2.880 + 240 = 20.120 EUR. 3) PREMIUM (Sauleda + 8cm Wolle, Kork-Plattform): 16k + 1k + 2.880 + 240 + 1.100 Sauleda + 400 8cm = 21.620 EUR. ACHTUNG Standard-Canvas haelt nur 3-5J in Algarve = alle 5J 1.400 EUR Plane-Tausch. Premium-Sauleda haelt 10-15J. CASHFLOW: Minimum 3.540 EUR/Mon machbar, Premium 4.155 EUR/Mon knapper."],
]
...
rows4 = [
    ...
    [3, "Atilla 8m Frame-only (Eiche+Oel) + Algarve-Sauleda-Sattler + Chris-Wolle",
     "DIY-Hybrid - HU Premium-Frame + Algarve-Service",
     "~18.270 EUR all-in (Atilla 7.350 + Versand 800 + Sauleda 2.500 + Wolle 1.000 + Solitex 700 + Innenliner 800 + Kuppel 800 + Schrauben 200 + Plattform Kork 2.880 + Sturm 500 + Schatten 240 + Reserve 500)",
     "PREIS-KOENIG fuer 8m DIY-Modify (Mai 2026): Atilla Frame-only-Angebot 7.350 EUR netto (6.600 Buche-Basis + 410 EICHE-Upgrade + 340 OEL-Behandlung) - Eiche+Oel ist STRUKTURELLER VORTEIL fuer Algarve (Klasse 2 Dauerhaftigkeit, atmungsaktiv, Termiten-resistent durch Gerbstoffe). Versand via Sebastians eigener Spediteur OK. 50 m² Wohnflaeche. 100 Std Eigenarbeit. ABER: Atilla liefert KEINE Acryl-Kuppel (Sebastian besorgt separat), KEIN Cover (Sebastian beauftragt Dune Algarve Sailmakers fuer Sauleda Solar Pro). Lieferzeit 2 Monate. Atilla seit 1996 etabliert, ehrlich+praezise.",
     "https://jurtak.hu/ | Sauleda lokal Algarve: https://www.dunesailmakers.com/"],

    [4, "DIY Modify Szenario A (Adorjan-Rahmen + Algarve-Sauleda-Sattler)",
    ...
    [5, "Paula Young - gebrauchte Casa-Jurte 7,3m",
    ...
    [6, "Yurt Made in Portugal (Loures, OLX-Hersteller)",
    ...
    [8, "Casa dos Sonhos Yurts (2027er Slot)",
...
log = [
    ...
    ["TBD", "Chris (PT-Schafwoll-Anbieter via Dani)", "Telegram (geplant)",
     "Sebastian -> Chris",
     "Erstanfrage: Selbst-Vorstellung mit Dani's Vermittlung als Referenz. Bedarf: 50-60 m² Schafwoll-Filz (8mm oder 16mm) fuer Algarve-Atlantikkueste. Fragen: Preis pro m², Lieferung 8650 Aljezur, Lieferzeit, Mengenrabatt, Motten-/Insekten-Behandlung, Algarve-Referenzkunden.",
     "⏳ Zu senden",
     "Bei Antwort: Vergleich mit Isolena AT (~16 EUR/m² netto) und Tobias Tumfart AT (~8 EUR/m² 8mm netto). Chris als lokale PT-Quelle koennte Versand-Vorteil haben."],
    ...
    ["24.05.2026 (Sebastians Folge)", "Mikael (Bento)", "Telegram DM",
     ...
     "✓ Gesendet", "Antwort 19:31"],
    ["24.05.2026 19:31", "Mikael (Bento)", "Telegram DM", "Mikael -> Sebastian",
     "WEITERE KLAERUNGEN: 1) Selbst-Abholung: nur Mietvan zahlen, kein Mikael-Aufpreis. 2) Vinyl ist 'vinyl coating' (Detail noch unklar). 3) Schrauben-Tausch durch Mikael: ER IST AUF REISEN, koennte jemand anderen organisieren. 4) Schrauben verkaufen wenn Sebastian sie nicht hat: ja. 5) LIEFERUNG ALJEZUR MINIMUM 1.000 EUR (van+sprit+maut+tageslohn) - nicht 500 wie geschaetzt! 6) Andere Pacific Yurts kosten ihn ~19.000 EUR. 7) WICHTIG MENSCHLICH: DRITTE TOCHTER DIESE WOCHE GEBOREN! Familien-Verkaufsdruck.",
     "✓ Klar - Lieferung teurer als gedacht",
     "Korrigierte Kalkulation: Mikael-Lieferung 1.000 (statt 500 geschaetzt), Selbst-Abholung 750. Diff nur 250 EUR fuer 3 Tage Aufwand. NEU: Vinyl-Frage klar definieren (coating vs alternativ-Dach). Schrauben selbst tauschen ist besser (Sebastian lernt Jurte kennen)."],
    ["24.05.2026 (Sebastians Folge)", "Mikael (Bento)", "Telegram DM",
     "Sebastian -> Mikael",
     "Empathische Antwort: Glueckwunsch zur Geburt der 3. Tochter. KLARER PLAN in 5 Punkten: 1) Visit Sa 14 oder 21 Juni mit Familie. 2) Selbst-Abholung mit Mietvan nach Besuch. 3) Schrauben Edelstahl A4 selbst kaufen in Algarve (Wuerth PT) + selbst tauschen vor Aufbau. 4) Vinyl-Frage: ist es Beschichtung auf Canvas (a) oder Alternativ-Dach (b)? Bei (a) ja nehmen, bei (b) lieber Canvas+Schattennetz. 5) Zusatz-Daemmung 8mm Schafwolle - Mikael Preis oder Chris PT?",
     "⏳ Zu senden", "Bei Antwort: Vinyl-Klaerung entscheidet ob +1.000 EUR sinnvoll oder besser Canvas+Schatten"],
    ["24.05.2026 - KRITISCH", "Sebastian Cashflow-Realitaet", "Selbst-Klaerung", "Sebastian intern",
     "WICHTIG: Sebastian hat aktuell nur 5.000 EUR sofort verfuegbar! Differenz ~16.000 EUR muss bis August angespart werden (~5.300 EUR/Monat). Realistische Frage: ist das machbar? Alternativen: A) Mikael Etappenzahlung 5/5/5/6k bis 15. August (transparente Bitte). B) Dani Custom-Bau passt natuerlich zum Bauzeit-Cashflow (Aug/Sep Fertigstellung). C) Konsumkredit PT-Bank 5-15k bei guter Bonitaet (5-7% Zinsen). D) Familien-Darlehen DE. E) Warten bis Sep/Okt mit mehr Eigenanteil.",
     "Strategischer Wendepunkt", "ENTSCHEIDET FINALE STRATEGIE: Sebastian sollte Mikael ehrlich Cashflow kommunizieren + Etappenzahlung 5/5/5/6 bis 15.08 vorschlagen. Lieferung erst nach Vollzahlung (Sicherheit fuer Mikael). Dani's Custom-Bau ist Plan B = passt naturgemaess zum Cashflow."],
    ["24.05.2026 spaeter", "Sebastian KORREKTUR-NOTIZ", "Selbst-Klaerung", "Sebastian intern",
     "Sebastian-Korrektur: Sauleda-Upgrade ist MEIN VORSCHLAG, Dani hat das NICHT aktiv angeboten. Sie sagte nur 'naehe gerne anderen Stoff wenn IHR ihn selbst besorgt'. Dani's BASIS ist 16k Standard mit BW/Poly Canvas + 4cm Wolle. 3 Varianten ALL-IN: MIN 19.165 EUR (EPS-Plattform), MITTEL 20.120 EUR (Kork-Plattform), PREMIUM 21.620 EUR (mit Sauleda+8cm).",
     "Klarstellung",
     "Korrigierte Cashflow-Bedarfe: MIN 3.540/Mon, MITTEL 3.780/Mon, PREMIUM 4.155/Mon - alle realistischer als Mikael 5.300/Mon."],
    ["~25.05.2026 (Sebastians naechste Mail)", "Dani (Castelo de Vide)", "Telegram DM",
     "Sebastian -> Dani",
     "INHALTLICHE 4-Punkte-Anfrage (OHNE Anzahlung-Thema - kommt spaeter separat): 1) AUFBAU WO? Bei euch in Castelo de Vide (ich hole ab) oder bei mir in Aljezur (Transport extra/inkl?)? 2) HOLZ-Behandlung Termiten was machen sie / was muss Sebastian nachbehandeln? 3) PLATTFORM-Masse fuer Sebastians DIY-Bau (Aussendurchmesser, Verankerung Khaana, Ueberstand)? 4) LIEFERZEIT konkret August oder September Woche X? Plus: PROBESCHLAFEN-Termin nach Mitte Juni?",
     "🔄 Umgeleitet an Chris (Danis Mann)",
     "Sebastian's Entscheidung: 4 inhaltliche Punkte direkt an Chris (Danis Mann, technisch-fokussierte Person, hat auch Schafwoll-Expertise). Eine Mail an Chris statt zwei separate Threads."],
    ["~25.05.2026", "Chris (Danis Mann, Castelo de Vide)", "Telegram DM",
     "Sebastian -> Chris",
     "FOLGE-MAIL (kein Selbst-Vorstellung mehr - bereits Kontakt). 5 Punkte: 1) AUFBAU - konkretisiert Danis Aussage 'inkl. Aufbau' (liefert ihr zu Aljezur? Transport extra?). 2) Holz-Behandlung Termiten. 3) Plattform-Masse fuer DIY. 4) Lieferzeit Aug/Sep konkret. 5) AUSSENHUELLE: Sebastian erwaehnt Marine-Acryl-Stoff-Recherche + 10-J-Garantie-Hinweis bei PT-Herstellern, fragt ob Alternative zum BW/Poly Standard moeglich (Aufpreis traegt Sebastian). Plus Probeschlafen-Termin.",
     "⏳ Zu senden",
     "Sebastians Recherche-Hinweis (Sauleda subtil) als ELEGANTES Anregen: Casa-dos-Sonhos-Referenz im Hintergrund ohne Wettbewerbsdruck-Gefuehl. 'Aufpreis traegt Sebastian' entwaffnet die Frage."],
    ["25.05.2026", "Atilla es a Fehernep", "E-Mail", "Atilla -> Sebastian",
     "KONKRETES FRAME-ONLY-ANGEBOT: Buche-Basis 6.600 + Eiche-Upgrade 410 + Oel-Behandlung 340 = 7.350 EUR netto. KEINE Acryl-Kuppel verfuegbar (nur mit seinem Cover kompatibel). Lieferzeit ~2 Monate. Versand: gleiches Auto noetig (kein Volumen-Rabatt), aber Sebastians EIGENER SPEDITEUR OK (Antwort positiv). Eiche besser als Laerche (in HU schwer zu bekommen). Oel-Behandlung statt Lack.",
     "✓ Klares konkretes Angebot",
     "ATILLA WIRD ZUM PREISKOENIG fuer DIY-Modify 8m: 18.270 EUR all-in mit Eiche+Oel+Sauleda+Wolle+Plattform DIY. 100 Std Eigenarbeit. Eiche+Oel = STRUKTURELL BESSER fuer Algarve als Pinie+Lack bei Mikael."],
    ["25.05.2026 (Sebastians Folge)", "Atilla es a Fehernep", "E-Mail",
     "Sebastian -> Atilla",
     "Klarstellungs-Fragen: 1) Was genau in 7.350 inkludiert (Khana? Tono? Uni? Tuer)? 2) Dimensionen-Standard 8m: Wandhoehe, Tono-Durchmesser, Uni-Anzahl+Querschnitt? 3) Timing: bei Bestellung Mitte Juni, wann Pickup ready? Plus: Kuppel besorgt Sebastian lokal mit Plane-Sattler.",
     "⏳ Zu senden",
     "Bei Antwort: finale Liste fuer Sebastian's Eigenkoordination der separaten Komponenten (Kuppel, Plane, Filz, Innenliner)"],
    ["25.05.2026", "Dani (Castelo de Vide)", "Telegram DM", "Dani -> Sebastian",
     "Chris hat ihr erzaehlt dass Sebastian Marinestoff von PT-Shop im Auge hat. Dani fragt: 'kannst du Link senden? Will pruefen ob meine Maschine das schafft (Limit 420g/m²)'",
     "✓ Stoff-Frage offen",
     "POSITIV: Dani technisch offen fuer Sauleda/Tempotest wenn Material kompatibel! Aber: Sebastian hat tatsaechlich noch keinen konkreten PT-Shop, nur Marken-Recherche. Muss ehrlich klarstellen."],
    ["25.05.2026 (Sebastians Folge)", "Dani (Castelo de Vide)", "Telegram DM",
     "Sebastian -> Dani",
     "Ehrliche Korrektur: 'Chris hat mich besser gemacht als ich bin - hab keinen festen Shop, nur Marken-Recherche.' Konkret: Sauleda Solar Pro/Nautic (290-340 g/m²), Tempotest Para Marine (290-310 g/m²) - beide klar unter 420g/m² Limit. PT-Direktshop keiner bekannt, aber Sauleda via spanischen Distributoren (Coartal, Toldum.es) oder via Dune Algarve Sailmakers Vilamoura. Dani's Lieferanten-Empfehlung erfragt. Demuetig: 'will euch nicht zwingen mit neuem Material'.",
     "✓ Gesendet",
     "Drei moegliche Reaktionen: a) Dani prueft Datenblatt = positiv, b) Dani kennt eigenen Lieferanten = Bonus, c) Dani bleibt bei ESVO Standard = Sebastian akzeptiert BW/Poly."],
    ["25.05.2026", "Dani (Castelo de Vide)", "Telegram DM", "Dani -> Sebastian",
     "Locker mit Humor ('aha 😜'): bestaetigt sie hat NUR ueber ESVO Holland bestellt+genaeht. ESVO liefert KOSTENLOS nach PT bei Menge. Bei neuem Material wuerde sie evtl. laenger zum Naehen brauchen, was Aufbau verschieben koennte - aber 'relativ schnell absehbar' wenn sie damit beginnt. Sebastian darf entscheiden was er will, sie unterstuetzt das.",
     "✓ Sehr kooperativ + ESVO-Hinweis",
     "GAME-CHANGER: ESVO hat Sauleda im Sortiment! Recherche bestaetigt: 'ESVO Holland is Europe's market leader outdoor fabrics, carries Sauleda as best acrylic fabric brand for boat marine'. Damit haben wir Sauleda bei DANI's BESTEHENDEM LIEFERANTEN + kostenloser PT-Lieferung."],
    ["25.05.2026 (Sebastians Folge)", "Dani (Castelo de Vide)", "Telegram DM",
     "Sebastian -> Dani",
     "Begeistert: 'ESVO hat ja Sauleda und andere Marine-Acryl im Programm!' Mit konkreten ESVO-Links (Sauleda + Boat-Canvas + Acrylic). Vorschlag: Premium-Variante ueber Danis gewohnten ESVO-Weg gehen (sie kennt Bestell-Prozedur, ESVO liefert kostenlos PT). Bittet Dani um EMPFEHLUNG welche ESVO-Marine-Variante fuer Algarve-Wohnjurte.",
     "✓ Gesendet",
     "Smart strategisch: Material-Wahl Dani uebergeben (sie kennt ESVO besser), Verantwortung Verarbeitung bei Dani. Direkte ESVO-Links als Verifikation."],
    ["25.05.2026", "Dani (Castelo de Vide)", "Telegram DM", "Dani -> Sebastian",
     "MARINESTOFF noch nie genaeht. Aber: 'CAMPSHIELD' einmal vernaeht - empfiehlt das anzuschauen bei ESVO.",
     "✓ Konkrete Material-Empfehlung",
     "TenCate Campshield = Glamping-Industriestandard, BW/Poly atmungsaktiv, inherent flammhemmend, in 290 g/m² oder 420 g/m² Varianten. PERFEKTE Mittelposition zwischen Standard BW/Poly Canvas und Premium Sauleda Acryl. Dani hat schon Erfahrung damit."],
    ["25.05.2026 (Sebastians Folge)", "Dani (Castelo de Vide)", "Telegram DM",
     "Sebastian -> Dani",
     "Recherche: TenCate Campshield = Glamping-Industriestandard, flammhemmend, 290 oder 420 g/m². 3 Fragen: 1) 290 oder 420 g/m² verwendet, wie hat sie sich gehalten? 2) Preis bei ESVO pro Meter / pro Jurte? 3) Algarve-Sonne: schwerere 420 g/m² empfehlen oder 290 g/m² reichen?",
     "✓ Gesendet",
     "Bei Antwort: konkrete Preise + Empfehlung."],
    ["25.05.2026", "Dani (Castelo de Vide)", "Telegram DM", "Dani -> Sebastian",
     "Kein direkter Vergleich leicht/schwer. Aber: hat damals den SCHWEREN 420 g/m² + UV-Schutz darueber verwendet (doppelter Schutz). Schwerer ist DOPPELTER PREIS gegenueber leichtem. Leichter 290 g/m² gerade im SALE in grau bei ESVO: https://www.esvocampingshop.com/de/outdoorstoffe-zeltstoff/merken-de/campshield-de/",
     "✓ Erfahrungs-Info + Sale-Link",
     "STRATEGISCHE OPTION: Da Sebastian SOWIESO Schattennetz fuer Algarve plant (240 EUR), reicht potentiell der LEICHTE 290 g/m² im Sale + Schattennetz = aequivalent zu Dani's damaliger Loesung. Spart ~700-1.000 EUR vs schwerer Variante."],
    ["25.05.2026 (Sebastians smarte Folge)", "Dani (Castelo de Vide)", "Telegram DM",
     "Sebastian -> Dani",
     "Logik-Frage: 'Schattennetz drueber plane ich fuer Algarve sowieso (UV+Sommer-Hitze) - reicht dann der leichte 290er nicht? Bei euch damals wars schwer+UV-Schutz = doppelt abgesichert.' Sale-Preis erwaehnt aber Dani's Erfahrung als Autoritaet anerkannt.",
     "✓ Gesendet",
     "Smart: zeigt Sebastian denkt selber mit. Antwort 27.05 erhalten."],
    ["27.05.2026 16:19", "Dani (Castelo de Vide)", "Telegram DM", "Dani -> Sebastian",
     "PREIS-KONKRETISIERUNG: Leichter Campshield 290 g/m² = ~1.400 EUR bei 7m-Jurte. (Standard BW/Poly Canvas via ESVO war auch ~1.400 EUR im 16k-Preis). Schwerer = doppelter Preis ~2.800 EUR. Sebastian fragt jetzt Klarstellung: Aufpreis fuer Campshield wegen Markenstoff oder gleichwertig im 16k?",
     "✓ Konkrete Preis-Info",
     "GAME-CHANGER: Leichter Campshield = vermutlich KEIN AUFPREIS! Gleicher Preis wie Standard-Canvas. Damit upgrade Sebastian ohne Mehrkosten von BW/Poly Standard auf TenCate Glamping-Marken-Stoff. Mit Schattennetz drueber Top-Setup. Klarstellungs-Mail an Dani sofort raus."],
    ["~28.05.2026 (Sebastians Klarstellung)", "Dani (Castelo de Vide)", "Telegram DM",
     "Sebastian -> Dani",
     "Klarstellungs-Frage: 'Verstehe ich das richtig - leichter Campshield 290 g/m² ersetzt einfach Standard-Canvas, also OHNE AUFPREIS im 16k? Wenn ja nehmen wir den.' Plus Bestaetigung dass Sebastian mit Schattennetz fuer Algarve gut aufgestellt ist + flammhemmende Markenqualitaet als Bonus.",
     "⏳ Zu senden",
     "Wenn Dani bestaetigt 'kein Aufpreis': finale Kalkulation Dani 20.620 EUR all-in (TOP-LOESUNG). Mikael verliert dann fast jeden Vorteil ausser 12m² Mehrflaeche."],
    ["~26.05.2026", "Mikael (Bento)", "Telegram DM", "Sebastian -> Mikael",
     "Ehrlich-empathische Cashflow-Mail: Entschuldigung fuer Verzoegerung, transparent ueber Cashflow (5k sofort + Rest bis Aug). Vorschlag Etappenzahlung 5/5/5/Rest mit Pickup Ende August. Anerkennt Mikaels Brand+Baby-Situation. Ausweg geben: 'wenn nicht moeglich, verkauf an wen schneller zahlen kann - kein hard feelings'. Erwaehnt parallel andere Optionen + Lob Mikaels Transparenz.",
     "✓ Gesendet", "Antwort 28.05 erhalten"],
    ["28.05.2026 12:16", "Mikael (Bento)", "Telegram DM", "Mikael -> Sebastian",
     "POSITIVE ANTWORT: 'no problem with that at all'. Schlaegt vor 5+5+5 = 15k (statt Sebastians 5+5+5+6 = 21k) - vermutlich nur Jurte-Basis, AC4+Vinyl+Versand extra. Drei Monate: jetzt + Juni + Juli. Pickup vermutlich Ende Juli nach Vollzahlung.",
     "✓ Etappenzahlung akzeptiert",
     "ABER STRATEGISCH: Sebastian noch nicht entschieden Mikael vs Dani. 5k blind anzuzahlen vor Juni-Besuch+Rost-Verifikation+Dani-Vergleich ist Risiko. Sollte um Verschiebung der ersten Rate auf Ende Juni (nach Besuch) bitten."],
    ["~29.05.2026 (Sebastians Folge)", "Mikael (Bento)", "Telegram DM",
     "Sebastian -> Mikael",
     "Bedankt sich + akzeptiert 5/5/5-Schema. Bittet um Verschiebung der ersten 5k-Anzahlung auf NACH dem Juni-Besuch (Ende Juni statt jetzt). Begruendung: 'fuehlt sich nicht ganz richtig an, blind anzuzahlen vor Treffen'. Konkretes Schema: Ende Juni 5k + Mitte Juli 5k + Anfang August 5k + Pickup Mitte August. Ausweg geben: 'wenn Geld frueher gebraucht, sag Bescheid'. Konkrete Termin-Frage: Sa 14 oder 21 Juni?",
     "✓ Gesendet",
     "Antwort 28.05 16:41 erhalten"],
    ["28.05.2026 16:41", "Mikael (Bento)", "Telegram DM", "Mikael -> Sebastian",
     "VERKAUFSDRUCK: 'I need you to give me a guarantee that you want this; I have many people asking for information about this yurt.' Klassischer Drucker-Move - kann wahr oder Bluff sein. Mikael will Sicherheit ueber Geld/Commitment.",
     "✓ Druck-Move",
    ...
    ["~29.05.2026 (Sebastian-Entscheidung)", "Mikael (Bento)", "Telegram DM",
     "Sebastian -> Mikael",
     "OPTION A (empfohlen): 2.000 EUR Reservierungs-Anzahlung sofort. Kompromiss: Mikael bekommt Liquiditaet + Commitment. Sebastian's Risiko 2k begrenzt. Bei Vertragsabschluss nach Besuch: 2k zaehlt zu 15k Total. Bei Rueckzug nach Besuch: Mikael behaelt 2k als Reservierungsgebuehr. Dann 5+5+3 = 15k bis Mitte August. Falls Mikael 2k nicht reicht: respektvoller Rueckzug mit 'no hard feelings'.",
     "⏳ Zu entscheiden",
     "ALTERNATIVE B: Kein Geld vor Besuch + Mikael akzeptiert wenn er an anderen verkauft. Sebastian's persoenliche Risiko-Bereitschaft entscheidet."],
